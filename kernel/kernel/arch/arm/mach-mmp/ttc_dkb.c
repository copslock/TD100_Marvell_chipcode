...
static unsigned long ttc_dkb_pin_config[] __initdata = {
	...
	/*keypad*/
#if defined(CONFIG_TD100_JLJ)
	GPIO00_KP_MKIN0,//0 row
	GPIO01_KP_MKOUT0,//0 col
	GPIO02_KP_MKIN1,//1 row
	GPIO03_KP_MKOUT1,//1 col
	GPIO05_KP_MKOUT2,//2 col
#else
	GPIO00_KP_MKIN0,
	GPIO01_KP_MKOUT0,
	GPIO02_KP_MKIN1,
...
#if defined(CONFIG_PXA910_CAMERA) || defined(CONFIG_VIDEO_MV)
...
#if defined(CONFIG_TD100_JLJ)
static unsigned int ttc_dkb_matrix_key_map[] = { /* 2(rows)x3(cols) matrix, row, col, value   */
	KEY(0, 0, KEY_CAMERA_FOCUS),
	KEY(0, 1, KEY_CAMERA),
	KEY(1, 1, KEY_HOME),
			...
			struct pm860x_platform_data *pdata)
{
	...
	*/
	...
	/* set vbuck2(power supply to DDR) to 1.8V for 920 */
	if (!emmc_boot) {
	...
	if (emmc_boot)
		...
		/* enable LDO13 for charge pump, 1.8V*/
		pm860x_reg_write(chip->client, PM8607_VIBRA_SET, 0x5);
#else
...
#ifdef CONFIG_TD100_JLJ
		//pm860x_reg_write(chip->client, PM8607_LDO3,0x3F); //set LDO3 3.3V for lcd & touchscreen,but rear camera vdd 2.8v~3.0v
#endif
...
#if defined(CONFIG_TD100_JLJ)
#if 0
static int rear_sensor_power_onoff(int on, int sensor)
{
	unsigned int cam_pwr;
	unsigned int cam_reset;

	/* actually, no small power down pin needed */
	cam_pwr = mfp_to_gpio(MFP_PIN_GPIO65);
	cam_reset = mfp_to_gpio(MFP_PIN_GPIO66);
...
static struct sensor_platform_data ov5642_sensor_data = {
	.id = SENSOR_HIGH,
	.power_on = rear_sensor_power_onoff,
};
#endif
static unsigned int cam_pdn_rear = 0;
static unsigned int cam_reset = 0;
static unsigned int cam_rst_front = 0;


static int request_cam_gpio(void)
{
	/* actually, no small power down pin needed */
	if(cam_rst_front == 0)
	    cam_rst_front = mfp_to_gpio(MFP_PIN_GPIO64);
	if(cam_pdn_rear == 0)
	    cam_pdn_rear  = mfp_to_gpio(MFP_PIN_GPIO65);
	if(cam_reset == 0)
	    cam_reset        = mfp_to_gpio(MFP_PIN_GPIO66);

	if (cam_pdn_rear)
		if (gpio_request(cam_pdn_rear, "CAM_PWR_MAIN")) {
			printk(KERN_ERR "Request GPIO failed,\
					gpio: %d\n", cam_pdn_rear);
			return -EIO;
		}
	if (cam_rst_front)
		if (gpio_request(cam_rst_front, "CAM_PWR_SUB")) {
			printk(KERN_ERR "Request GPIO failed,\
					gpio: %d\n", cam_rst_front);
			return -EIO;
		}
...
static int free_cam_gpsio(void)
{
       if (cam_pdn_rear)
           gpio_free(cam_pdn_rear);
	if (cam_reset)
	    gpio_free(cam_reset);
	if (cam_rst_front)
	    gpio_free(cam_rst_front);

       return 0;
}

static int set_subcam_gpios_low(void)
{
    static unsigned int cam_vsyn_front = 0;
    static unsigned int cam_hsyn_front = 0;
    static unsigned int cam_pclk_front = 0;

     cam_vsyn_front = mfp_to_gpio(MFP_PIN_GPIO76);
     if (cam_vsyn_front)
		if (gpio_request(cam_vsyn_front, "CAM_VSYN_SUB")) {
			printk(KERN_ERR "Request GPIO failed,\
					gpio: %d\n", cam_vsyn_front);
			goto SUBCAM_FREE_GPIOS;
		}

     cam_hsyn_front  = mfp_to_gpio(MFP_PIN_GPIO75);
     if (cam_hsyn_front)
		if (gpio_request(cam_hsyn_front, "CAM_HSYN_SUB")) {
			printk(KERN_ERR "Request GPIO failed,\
					gpio: %d\n", cam_hsyn_front);
			goto SUBCAM_FREE_GPIOS;
		}

     cam_pclk_front   = mfp_to_gpio(MFP_PIN_GPIO78);
     if(cam_pclk_front)
	       if (gpio_request(cam_pclk_front, "CAM_RESET")) {
		printk(KERN_ERR "Request GPIO failed, gpio: %d\n", cam_pclk_front);
		goto SUBCAM_FREE_GPIOS;
	}

       gpio_direction_output(cam_vsyn_front, 0);
	gpio_direction_output(cam_hsyn_front, 0);
	gpio_direction_output(cam_pclk_front, 0);

SUBCAM_FREE_GPIOS:
       if (cam_vsyn_front)
           gpio_free(cam_vsyn_front);
	if (cam_hsyn_front)
	    gpio_free(cam_hsyn_front);
	if (cam_pclk_front)
	    gpio_free(cam_pclk_front);

       return 0;
}
static unsigned long myCAM_GPIO[14];
static void pxa910_cam_pins_power(int on)
{
	unsigned int pin_index = 0, i = 0;
	/*on=0, power save*/
	if (!on) {
		for (pin_index = MFP_PIN_GPIO67; pin_index <= MFP_PIN_GPIO80; pin_index++)
			myCAM_GPIO[i++] = mfp_read(pin_index);
		...
		/* restore pin value*/
		for (pin_index = MFP_PIN_GPIO67; pin_index <= MFP_PIN_GPIO80; pin_index++)
			mfp_write(pin_index, myCAM_GPIO[i++]);
	}
}

extern int mt9v113_enter_standby(void);
...
static int rear_sensor_power_onoff(int on, int sensor)
{
	...
	if (on) {
              //front camera standby,
	       gpio_direction_output(cam_rst_front, 1);
	       mdelay(10);
	       mt9v113_enter_standby(); //mclk must on before do it.
	       mdelay(10);
	       gpio_direction_output(cam_rst_front, 0);
		//pxa910_cam_pins_power(0);
              //mfp_config(ARRAY_AND_SIZE(ccic_as_pins_config));
	       //set_subcam_gpios_low();

		msleep(1);
		gpio_direction_output(cam_pdn_rear, 0);
		msleep(1);
		gpio_direction_output(cam_reset, 0);
		msleep(10);
		gpio_direction_output(cam_reset, 1);
		...
		gpio_direction_output(cam_pdn_rear, 1);
		gpio_direction_output(cam_reset, 0);
	}

	msleep(10);
//	printk("rear_sensor_power_onoff(%d,%d)\n",on,sensor);
	return 0;
}
...
/*modify for frontt camera by liyong*/
static int front_sensor_power_onoff(int on, int sensor)
{
       if(0 == cam_rst_front)
		return -EIO;

	if (on) {
              //main/rear camera standby
              //pxa910_cam_pins_power(1);
	...
	msleep(10);
  //     printk("front_sensor_power_onoff(%d,%d)\n",on,sensor);
	return 0;
...
static struct sensor_platform_data mt9v113_sensor_data ={
	.id = SENSOR_LOW,
	//.id = SENSOR_HIGH,
	.power_on =front_sensor_power_onoff,
};
#endif
...
struct mv_cam_pdata mv_cam_data = {
	...
	.clk_init = pxa910_cam_clk_init,
	.set_clock = pxa910_cam_set_clk,
...
{
	unsigned int tp_reset;/*enable 1,disable  0 */
	unsigned int tp_wake;/* bootup ,need set 1 */

	tp_reset = mfp_to_gpio(MFP_PIN_GPIO8);
	tp_wake = mfp_to_gpio(MFP_PIN_GPIO11);

	if (gpio_request(tp_reset, "TP_RESET")) {
		printk(KERN_ERR "TP RESET Request GPIO failed,"
		       "gpio: %d \n", tp_reset);
		return -EIO;
	}

	if (gpio_request(tp_wake, "TP_WAKE")) {
		printk(KERN_ERR "TP WAKE Request GPIO failed,"
		       "gpio: %d \n", tp_wake);
		return -EIO;
	}

	if (on) {
		gpio_direction_output(tp_reset, 1);
		gpio_direction_output(tp_wake, 1);
	} else {
		gpio_direction_output(tp_reset, 0);
		gpio_direction_output(tp_wake, 0);
	}
...
static void ft5306_touch_reset(void)
{
	...
	gpio_direction_output(MFP_PIN_GPIO8, 0);
	mdelay(5);
	gpio_direction_output(MFP_PIN_GPIO8, 1);
...
#ifdef CONFIG_SENSORS_SFH7773
static struct bh1770glc_platform_data sfh7773_pdata = {
	.leds = BH1770GLC_LED1,
	.led_max_curr = BH1770GLC_LED_200mA,
/*    .led_def_curr = 0,*/
};
#endif

#ifdef CONFIG_SENSORS_L3G4200
...
static struct tps6105x_platform_data tps61050_pdata = {
	.mode = TPS6105X_REG0_MODE_TORCH_FLASH,
	.regulator_data = NULL,
};
#endif
...
#endif
#if 1
#if defined(CONFIG_SENSORS_SFH7773)
	{
		.type		= "bh1770glc",
		.addr		=  0x38,
		.irq		= gpio_to_irq(9),
		.platform_data	= &sfh7773_pdata,
	},
#endif
#endif
...
#endif
#if 1
#if defined(CONFIG_SENSORS_L3G4200)
    {
        .type      = "l3g4200d_gyr",
        .addr      =  0x68,
...
#endif
#endif
#if defined(CONFIG_TPS61050)
    {
	.type      = "tps6105x",
	.addr      =  0x33,
	.platform_data    = &tps61050_pdata,
    },
#endif
...
#endif
#if defined(CONFIG_VIDEO_PXA910_OV5640)
	{
		.type		= "ov5640",
		/* Address is 0x3c, but conflict with 3640 */
		.addr           = 0x3e,
		.platform_data  = &ov5642_sensor_data,//&ov3640_sensor_data,
	},
#endif
};
#else
...
#ifdef CONFIG_TD100_JLJ
static int wvga_lcd = 1;//default choose truly panel
#else
...
int mmc1_idle_switch(u32 on)
{
	...
	if (!*pxa910_sdh_platdata_mmc1.pmmc
		|| !(*pxa910_sdh_platdata_mmc1.pmmc)->card
		|| (*pxa910_sdh_platdata_mmc1.pmmc)->suspended)
		return 0;
#endif
...
static int mmc0_lp_switch(unsigned int on, int with_card)
{
	...
	/* Clock pin is floating by default, so enable internal PULL-UP of clock pin
	 * during suspend to save suspend power if SD power is kept always on.
	 */
...
{
	static int WIB_3V3_EN = 0;
	static int WLAN_BT_PWR_EN = 0;/* 1.8V RF power */

	if (!WIB_3V3_EN || !WLAN_BT_PWR_EN) {
		if (is_td_dkb) {
			WIB_3V3_EN = mfp_to_gpio(MFP_PIN_GPIO10);
			WLAN_BT_PWR_EN = mfp_to_gpio(MFP_PIN_GPIO17);
		}

		if (gpio_request(WIB_3V3_EN, "WIB_3V3_EN")) {
			printk(KERN_INFO "gpio %d request failed\n", WIB_3V3_EN);
			WIB_3V3_EN = WLAN_BT_PWR_EN = 0;
			return;
		}
		if (gpio_request(WLAN_BT_PWR_EN, "WLAN_BT_PWR_EN")) {
			printk(KERN_INFO "gpio %d request failed\n", WLAN_BT_PWR_EN);
			gpio_free(WIB_3V3_EN);
			WIB_3V3_EN = WLAN_BT_PWR_EN = 0;
			return;
		}

#ifdef CONFIG_GPIO_SYSFS
		gpio_export(WIB_3V3_EN, false);
		gpio_export(WLAN_BT_PWR_EN, false);
#endif
	}
	BUG_ON(!WIB_3V3_EN || !WLAN_BT_PWR_EN);

	pr_debug("%s: on=%d\n", __FUNCTION__, on);
	if (on) {
		if (WIB_3V3_EN) gpio_direction_output(WIB_3V3_EN, 1);
		if (WLAN_BT_PWR_EN) gpio_direction_output(WLAN_BT_PWR_EN, 1);
	} else {
		if (WIB_3V3_EN) gpio_direction_output(WIB_3V3_EN, 0);
		if (WLAN_BT_PWR_EN) gpio_direction_output(WLAN_BT_PWR_EN, 0);
	}
}
#else
static void ttc_dkb_wifi_set_power(unsigned int on)
...
static void __init pxa910_init_mmc(void)
{
	...
	if(is_td_dkb) {
#ifdef CONFIG_TD100_JLJ
		WIB_PDn = mfp_to_gpio(MFP_PIN_GPIO16);
		WIB_RESETn = mfp_to_gpio(MFP_PIN_GPIO13);
#else
		WIB_PDn = GPIO_EXT1(0);
...
static int ttc_pin_lpm_config(void)
{
	...
	for (pin_index   = 21; pin_index  < 25; pin_index ++) {
		mfp_write(pin_index, 0x10c0);
	}
...
#if defined(CONFIG_TD100_JLJ)
	gps_ldo = (is_td_dkb) ? mfp_to_gpio(MFP_PIN_GPIO45) : GPIO_EXT1(7);
#else
...
#if defined(CONFIG_TD100_JLJ)
	gps_rst_n = (is_td_dkb) ? mfp_to_gpio(MFP_PIN_GPIO49) : mfp_to_gpio(MFP_PIN_GPIO15);
#else
...
#if defined(CONFIG_TD100_JLJ)
	gps_on = (is_td_dkb) ? mfp_to_gpio(MFP_PIN_GPIO46) : GPIO_EXT1(1);
#else
...
#if defined(CONFIG_TD100_JLJ)
	cmmb_rst = mfp_to_gpio(MFP_PIN_GPIO18);
#else
...
#if defined(CONFIG_TD100_JLJ)
	cmmb_en = mfp_to_gpio(MFP_PIN_GPIO19);
#else
	cmmb_en = GPIO_EXT1(6);
#endif
...
#if defined(CONFIG_TD100_JLJ)
	.gpio_power = mfp_to_gpio(MFP_PIN_GPIO19),
	.gpio_reset = mfp_to_gpio(MFP_PIN_GPIO18),
#else
...
static void __init ttc_dkb_init(void)
{
...
#if defined(CONFIG_PXA910_CAMERA) || defined(CONFIG_VIDEO_MV)
    #if defined(CONFIG_TD100_JLJ)
       request_cam_gpio(); // by Gallon.Qin 2012-02-17
    #endif
#endif
	/* off-chip devices */
	...
	/*	modify i2c devices rear camera, tp, magnetormeter to power i2c bus */
