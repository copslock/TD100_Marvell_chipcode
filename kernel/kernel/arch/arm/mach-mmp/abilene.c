...
static void pxa620_cam_ctrl_power(int on)
{
	return;
}

static int pxa620_cam_clk_init(struct device *dev, int init)
{
...
static void pxa620_cam_set_clk(struct device *dev, int on)
{
	struct mv_cam_pdata *data = dev->platform_data;
	if (on) {
...
struct mv_cam_pdata mv_cam_data = {
	...
	.mclk_min = 26,
	.mclk_src = 3,
	.controller_power = pxa620_cam_ctrl_power,
	.clk_init = pxa620_cam_clk_init,
	.set_clock = pxa620_cam_set_clk,
...
MACHINE_START(ABILENE, "Abilene Development Platform")
	.map_io		= pxa_map_io,
