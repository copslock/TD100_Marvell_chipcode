...
{
	int len, i;
	unsigned int total_ticks, result, fraction;
	...
	for (i = 0; i < (event_num / 2); i++) {
		...
		result   = (op_ticks_array[i].runtime * 100) / total_ticks;
		fraction = (op_ticks_array[i].runtime * 100) % total_ticks;
		fraction = fraction * 100 / total_ticks;
		len += sprintf(buf + len, "run:%2u.", result);
		if (fraction < 10)
			len += sprintf(buf + len, "0");
		len += sprintf(buf + len, "%u%%   ", fraction);

		result   = (op_ticks_array[i].idletime * 100) / total_ticks;
		fraction = (op_ticks_array[i].idletime * 100) % total_ticks;
		fraction = fraction * 100 / total_ticks;
