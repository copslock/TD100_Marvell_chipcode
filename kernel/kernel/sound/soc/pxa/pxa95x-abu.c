...
static int pxa95x_abu_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	struct abu_ssp_device *p_abu_ssp_dev = NULL;
	...
	if ((!cpu_dai->active) && (!p_abu_runtime_ctx->abu_ref_cnt)) {
		...
		/* enable abu function clock & SSP function clock */
		ret = clk_enable(p_abu_runtime_ctx->abu_clk);
		if (ret) {
			pr_err("[ABU] failed in clk_enable (abu clock) \r\n");
			goto func_exit;
		}

		p_abu_ssp_dev = &p_abu_runtime_ctx->abu_ssp_dev;
		ssp = p_abu_ssp_dev->ssp;

		ret = clk_enable(ssp->clk);
		if (ret) {
			pr_err("[ABU] failed in clk_enable (ssp->clk) \r\n");
			goto func_exit;
		}

		/* disable D2/CG before ABU running */
...
static void pxa95x_abu_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if ((!cpu_dai->active) && (1 == p_abu_runtime_ctx->abu_ref_cnt)) {
		...
		/* disable abu function clock & SSP clock */
		clk_disable(p_abu_runtime_ctx->abu_clk);
		clk_disable(ssp->clk);

		/* reenable D2/CG after ABU stop */
...
static int pxa95x_abu_suspend(struct snd_soc_dai *cpu_dai)
{
	struct abu_device_ctx *p_abu_runtime_ctx = cpu_dai->private_data;

	pr_debug("[ABU] pxa95x_abu_suspend  ++++++++++++++++++ \r\n");
	if (p_abu_runtime_ctx->abu_ref_cnt >= 1)
		pxa95x_abu_unset_dvfm_constraint(p_abu_runtime_ctx->abu_ssp_dvfm_idx);

	return 0;
}

static int pxa95x_abu_resume(struct snd_soc_dai *cpu_dai)
{
	struct abu_device_ctx *p_abu_runtime_ctx = cpu_dai->private_data;

	pr_debug("[ABU] pxa95x_abu_resume  ++++++++++++++++++ \r\n");
	if (p_abu_runtime_ctx->abu_ref_cnt >= 1)
		pxa95x_abu_set_dvfm_constraint(p_abu_runtime_ctx->abu_ssp_dvfm_idx);
	return 0;
}
...
static int pxa95x_abu_trigger(struct snd_pcm_substream *substream, int cmd, struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		if (ABU_PLAYBACK_ONLY_STATUS == p_abu_runtime_ctx->abu_status)
...
{
	{
		...
		.suspend = pxa95x_abu_suspend,
		.resume = pxa95x_abu_resume,
		.playback = {
			.channels_min = 1,
			.channels_max = 2,
