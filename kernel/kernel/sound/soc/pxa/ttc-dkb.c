...
#define PM860X_LOFI_RATES (SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_16000 | \
				SNDRV_PCM_RATE_32000 | SNDRV_PCM_RATE_48000)

static int ttc_pm860x_lofi_startup(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai_link *machine = rtd->dai;
	struct snd_soc_dai *cpu_dai = machine->cpu_dai;
	...
	/* support 8k,16k,32k,48k */
	cpu_dai->playback.rates = PM860X_LOFI_RATES;
	cpu_dai->capture.rates = PM860X_LOFI_RATES;

	return 0;
}

static int ttc_pm860x_lofi_prepare(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai_link *machine = rtd->dai;
	struct snd_soc_dai *cpu_dai = machine->cpu_dai;
...
{
	.startup = ttc_pm860x_lofi_startup,
	.prepare = ttc_pm860x_lofi_prepare,
},
};
...
static int ttc_pm860x_init(struct snd_soc_codec *codec)
{
	 ...
	 * for SSP unit */
	*(volatile unsigned long *)MPMU_ISCCRX0 = 0xf820130b;
