...
static int mmp2_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if (base_register) {
		PXA688_ADCR(base_register) = (prtd->params->dcmd)
		    & (~SDCR_CHANEN);
		PXA688_ADIMR(base_register) = SDIMR_COMP;
	} else
		ret = -EINVAL;
...
static int mmp2_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:
		PXA688_ADNDPR(base_register) = prtd->squ_desc_array_phys;
		PXA688_ADCR(base_register) = prtd->params->dcmd | SDCR_CHANEN;
...
static int mmp2_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		snd_soc_set_runtime_hwparams(substream, &mmp2_pcm_hardware_playback);
	else
		snd_soc_set_runtime_hwparams(substream, &mmp2_pcm_hardware_capture);
...
void mmp2_save_adma_regs(struct snd_pcm_substream *substream)
{
	...
	prtd->pxa688_adma_saved[ch - 2].src_addr = PXA688_ADSAR(base_register);
	prtd->pxa688_adma_saved[ch - 2].dest_addr = PXA688_ADDAR(base_register);
	prtd->pxa688_adma_saved[ch - 2].next_desc_ptr =
	    PXA688_ADNDPR(base_register);
	prtd->pxa688_adma_saved[ch - 2].ctrl = PXA688_ADCR(base_register);
	if (ch == 2 || ch == 4) {
		prtd->pxa688_adma_saved[ch - 2].chan_pri =
		    PXA688_ADCP(base_register);
	}
	prtd->pxa688_adma_saved[ch - 2].curr_desc_ptr =
	    PXA688_ADCDPR(base_register);
	prtd->pxa688_adma_saved[ch - 2].intr_mask = PXA688_ADIMR(base_register);
	prtd->pxa688_adma_saved[ch - 2].intr_status =
	    PXA688_ADISR(base_register);
...
void mmp2_restore_adma_regs(struct snd_pcm_substream *substream)
{
	...
	PXA688_ADIMR(base_register) = prtd->pxa688_adma_saved[ch - 2].intr_mask;
	PXA688_ADCR(base_register) = prtd->pxa688_adma_saved[ch - 2].ctrl;
