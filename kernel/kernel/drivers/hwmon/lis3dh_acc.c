...
#if 0
    printk("%s read hw_d[0] = %d, hw_d[1] = %d, hw_d[2] = %d\n", LIS3DH_ACC_DEV_NAME, hw_d[0], hw_d[1], hw_d[2]);

    printk( "%s read hw_d[1]=%d, hw_d[2]=%d, hw_d[3]=%d\n",
			LIS3DH_ACC_DEV_NAME, hw_d[0], hw_d[1], hw_d[2]);
    printk( "%s acc->pdata->negate_x=%d", LIS3DH_ACC_DEV_NAME, acc->pdata->negate_x);
#endif
...
#if 0
        for (i = 0; i < 3; i++) {
            xyz[i] = hw_d[i] * GRAVITY_EARTH / LIS3DH_RESOLUTION;
        }
#endif
	#ifdef DEBUG
        /*printk( "%s read acc->pdata->axis_map_x=%d, acc->pdata->axis_map_y=%d, acc->pdata->axis_map_z=%d\n",
			LIS3DH_ACC_DEV_NAME, acc->pdata->axis_map_x, acc->pdata->axis_map_y, acc->pdata->axis_map_z);*/
...
static int lis3dh_get_position(struct device *dev)
{
	...
	struct lis3dh_acc_data *lis3dh = input_get_drvdata(input);

	return atomic_read(&lis3dh->position);
}

static void lis3dh_set_position(struct device *dev, unsigned long position)
{
	struct input_dev *input = to_input_dev(dev);
	struct lis3dh_acc_data *lis3dh = input_get_drvdata(input);
...
static DEVICE_ATTR(interval, S_IRUGO | S_IWUGO,
		   attr_get_polling_rate, attr_get_polling_rate);
static DEVICE_ATTR(position, S_IRUGO | S_IWUGO,
		   lis3dh_position_show, lis3dh_position_store);
static DEVICE_ATTR(wake, S_IWUGO, NULL, lis3dh_wake_store);
...
#if 1
         err =
	    sysfs_create_group(&acc->input_dev->dev.kobj,
