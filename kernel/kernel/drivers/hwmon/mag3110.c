...
static void report_abs(void)
{
	...
	printk("mag3110 x=%d,y=%d,z=%d.\n", x, y, z);

/*    idev = mag3110_pdata->poll_dev->input;*/
	idev = mag3110_pdata->input;
...
static void mag3110_work_func(struct work_struct *work)
{
	struct mag3110_data *mag3110 =
	    container_of((struct delayed_work *)work,
			 struct mag3110_data, work);
	unsigned long delay = atomic_read(&mag3110->delay);

	report_abs();


	schedule_delayed_work(&mag3110->work, delay);
...
/*    struct i2c_client *client = to_i2c_client(dev);*/
	...
	struct mag3110_data *mag3110 = input_get_drvdata(input);

	val = mag3110_read_reg(mag3110->client, MAG3110_CTRL_REG1) & MAG3110_AC_MASK;
...
/*    client = to_i2c_client(dev);*/
	...
	delay = atomic_read(&mag3110->delay);

	reg = mag3110_read_reg(mag3110->client, MAG3110_CTRL_REG1);
	if (enable)
		reg |= MAG3110_AC_MASK;
	else
		reg &= ~MAG3110_AC_MASK;
	...
	ret = mag3110_write_reg(mag3110->client, MAG3110_CTRL_REG1, reg);
	if (ret < 0)
		return ret;
	...
	if (enable) {
		...
		/* Read out MSB data to clear interrupt flag automatically */
		 mag3110_read_block_data(mag3110->client, MAG3110_OUT_X_MSB,
					 MAG3110_XYZ_DATA_LEN, tmp_data);

		 if (!atomic_cmpxchg(&mag3110->enable, 0, 1)) {

			 atomic_set(&mag3110->enable, 1);
			 schedule_delayed_work(&mag3110->work,
					 msecs_to_jiffies(delay) + 1);
		 }

	}
	else{
	if (atomic_cmpxchg(&mag3110->enable, 1, 0))
		atomic_set(&mag3110->enable, 0);
		cancel_delayed_work_sync(&mag3110->work);
	}
...
#else
static DEVICE_ATTR(active, S_IWUGO | S_IRUGO,
		   mag3110_enable_show, mag3110_enable_store);
#endif
...
#else
static DEVICE_ATTR(interval, S_IWUGO | S_IRUGO,
		   mag3110_dr_mode_show, mag3110_dr_mode_store);
#endif
				   ...
				   const struct i2c_device_id *id)
{
	...
	atomic_set(&data->delay, POLL_INTERVAL);

	/* setup driver interfaces */
	INIT_DELAYED_WORK(&data->work, mag3110_work_func);
	...
	if (!input_data) {
		ret = -ENOMEM;
		printk(KERN_ERR
		       "mag3110_probe: Failed to allocate input_data device\n");
		goto error_kfree;
	}

	input_data->name = MAG3110_DRV_NAME;
	input_data->id.bustype = BUS_I2C;
	input_data->evbit[0] = BIT_MASK(EV_ABS);
	input_data->dev.parent = &data->client->dev;

	input_set_abs_params(input_data, ABS_X, -15000, 15000, 0, 0);
	input_set_abs_params(input_data, ABS_Y, -15000, 15000, 0, 0);
	input_set_abs_params(input_data, ABS_Z, -15000, 15000, 0, 0);
	...
	ret = sysfs_create_group(&data->input->dev.kobj, &mag3110_attr_group);
	if (ret) {
	...
	printk("DBJtech mag3110 succeed\n");
	return 0;
