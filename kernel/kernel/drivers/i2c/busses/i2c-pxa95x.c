 ...
 *		- CUSTOMIZATION_PER_SLAVE_DEVICE: SLOW, i2c_pxa_custom_scream_silent(), CUSTOM_BYTE2BYTE_DELAY
...
#define i2c_pxa_custom_scream_silent(I2C)   (0)
...
*/
static int i2c_pxa_custom_scream_silent(struct pxa_i2c *i2c)
{
...
static int i2c_pxa_do_xfer_with_fifo(struct pxa_i2c *i2c, struct i2c_msg *msg, int num)
{
	unsigned long cpsr;
	u32 icr = 0, isr, ibmr;
	u32 wfifo_wptr,wfifo_rptr;
	u32 rfifo_wptr,rfifo_rptr;
	u32 retry;
	int ret;
	long timeout;
	...
	wfifo_wptr = readl(_WFIFO_WPTR(i2c));
	wfifo_wptr &= ~(0xf);
	writel(wfifo_wptr, _WFIFO_WPTR(i2c));

	wfifo_rptr = readl(_WFIFO_RPTR(i2c));
	wfifo_rptr &= ~(0xf);
	writel(wfifo_rptr, _WFIFO_RPTR(i2c));

	rfifo_wptr = readl(_RFIFO_WPTR(i2c));
	rfifo_wptr &= ~(0xf);
	writel(rfifo_wptr, _RFIFO_WPTR(i2c));

	rfifo_rptr = readl(_RFIFO_RPTR(i2c));
	rfifo_rptr &= ~(0xf);
	writel(rfifo_rptr, _RFIFO_RPTR(i2c));
...
static void i2c_pxa_irq_txempty_with_fifo(struct pxa_i2c *i2c, u32 isr)
{
	...
	if(isr & (ISR_BED)){
		...
		if(i2c_pxa_custom_scream_silent(i2c))
		{
			i2c->fifo_transaction_down = true;
			wake_up(&i2c->wait);
			return;
...
static void i2c_pxa_irq_txempty(struct pxa_i2c *i2c, u32 isr)
{
	...
	if ((isr & (ISR_BED | ISR_ALD)) == (ISR_BED | ISR_ALD)) {
...
*****/
	...
	if (i2c_pxa_is_slavemode(i2c)) {
		...
		if (i2c->flags & PXA_I2C_USING_FIFO_DMA_MODE) {
			...
			i2c_pxa_irq_txempty_with_fifo(i2c, isr);
		} else {
