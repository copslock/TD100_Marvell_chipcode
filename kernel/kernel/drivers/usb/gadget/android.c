...
int android_switch_function(char *func)
{
	struct usb_function		*f;
	struct android_dev *dev = _android_dev;
	int product_id;
	if (!func) {
		printk(KERN_ERR "illegal usb composite configuration string\n");
		return 0;
	}
	...
	list_for_each_entry(f, &android_config_driver.functions, list) {
		...
		} else if (strstr(func, "usb_mass_storage") &&
			!strcmp(f->name, "usb_mass_storage"))
			f->disabled = 0;
		else if (strstr(func, "diag") &&
			!strcmp(f->name, "diag"))
			f->disabled = 0;
		else
			f->disabled = 1;
