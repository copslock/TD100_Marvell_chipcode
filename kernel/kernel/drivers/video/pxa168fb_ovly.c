...
static int pxa168fb_release(struct fb_info *fi, int user)
{
	...
	x = (dma_ctrl_read(fbi->id, 0) & CFG_GRA_ENA_MASK);
	val = check_modex_active(fbi->id, 1);
	if (!x)
		dma_ctrl_set(fbi->id, 0, CFG_GRA_ENA_MASK, CFG_GRA_ENA(val));
...
again:
	...
	if (x_bk != x) {
		...
		pr_debug("set fbi %d dma ctrl0(0x%x -> 0x%x): addr %lu"
			" active %x fbi->active %d\n", fbi->id, x_bk,
			x, fbi->new_addr[0], active, fbi->active);
	}
...
static void set_yuv_pitch(struct pxa168fb_info *fbi, struct _sOvlySurface *surface, struct fb_var_screeninfo *var)
{
	...
	if (xzoom && var->bits_per_pixel) {
		...
		u32 reg = (fbi->id == 2) ? LCD_PN2_LAYER_ALPHA_SEL1 : LCD_AFA_ALL2ONE;
