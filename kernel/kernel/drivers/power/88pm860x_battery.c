...
#if 0
#define TBAT_HC_0D			1589	/* 0 */
#define TBAT_HC_40D			80	/* 40 */
#define TBAT_HC_42D	   		75	/* 42 */
#define TBAT_HC_45D			70	/* 45 */
#define TBAT_HC_55D			60	/* 55 */
#define TBAT_HC_57D	  		50	/* 57 */
#define TBAT_HC_60D			40	/* 60 */
#define TBAT_HC_100D			20	/* 100 */
#endif

#define TBAT_HC_0D			1589	/* 0 */
#define TBAT_HC_40D			30	/* 40 */
#define TBAT_HC_42D	   		25	/* 42 */
#define TBAT_HC_45D			20	/* 45 */
#define TBAT_HC_55D			15	/* 55 */
#define TBAT_HC_57D	  		10	/* 57 */
#define TBAT_HC_60D			5	/* 60 */
#define TBAT_HC_100D			3	/* 100 */
...
int array_soc[][2] = {  {4180, 100},
                            {4154, 99}, {4129, 98}, {4120, 97}, {4109, 96},
                            {4100, 95}, {4088, 94}, {4081, 93}, {4070, 92},
                            {4060, 91}, {4053, 90}, {4044, 89}, {4035, 88},
                            {4026, 87}, {4018, 86}, {4008, 85}, {4000, 84},
                            {3990, 83}, {3980, 82}, {3972, 81}, {3964, 80},
                            {3958, 79}, {3951, 78}, {3944, 77}, {3937, 76},
                            {3929, 75}, {3922, 74}, {3916, 73}, {3910, 72},
                            {3903, 71}, {3897, 70}, {3890, 69}, {3883, 68},
                            {3876, 67}, {3870, 66}, {3863, 65}, {3856, 64},
                            {3850, 63}, {3844, 62}, {3839, 61}, {3834, 60},
                            {3830, 59}, {3824, 58}, {3818, 57}, {3815, 56},
                            {3810, 55}, {3805, 54}, {3801, 53}, {3798, 52},
                            {3793, 51}, {3790, 50}, {3787, 49}, {3783, 48},
                            {3780, 47}, {3777, 46}, {3774, 45}, {3770, 44},
                            {3768, 43}, {3765, 42}, {3762, 41}, {3759, 40},
                            {3757, 39}, {3755, 38}, {3753, 37}, {3751, 36},
                            {3749, 35}, {3747, 34}, {3745, 33}, {3744, 32},
                            {3742, 31}, {3740, 30}, {3738, 29}, {3736, 28},
                            {3734, 27}, {3732, 26}, {3729, 25}, {3726, 24},
                            {3723, 23}, {3720, 22}, {3717, 21}, {3713, 20},
                            {3710, 19}, {3705, 18}, {3700, 17}, {3697, 16},
                            {3691, 15}, {3686, 14}, {3681, 13}, {3675, 12},
                            {3670, 11}, {3665, 10}, {3661, 9},  {3657, 8},
                            {3653, 7},  {3648, 6},  {3643, 5},  {3637, 4},
                            {3627, 3},  {3595, 2},  {3555, 1},  {3450, 0} };
...
static void pm860x_changed_work(struct work_struct *work)
{
	...
	if (ret & STATUS2_BAT) {
		...
		if (ret < 0)
			goto out;
		mutex_lock(&info->lock);
		dev_dbg(info->dev, "pm860x_changed_work current : %d\n", data);
		info->present = 1;
		if (data > 0)
	...
	dev_dbg(info->dev, "pm860x_changed_work status : %d\n", info->status);
	power_supply_changed(&info->battery);
...
static void pm860x_init_battery(struct pm860x_battery_info *info)
{
	...
	//dev_dbg(info->dev,"battery present :%d \n", info->present);
	mutex_unlock(&info->lock);
...
//	dev_dbg(info->dev, "measure_current = %d.\n", *data);
	return 0;
}
...
static int calc_ocv(struct pm860x_battery_info *info, int *ocv)
{
	...
	if (info->present)
	{
//		dev_dbg(info->dev, "A calc_ocv present : %d.\n", info->present);
		*ocv = vbatt_avg - ibatt_avg * info->resistor / 1000;
	}
	else
	{
//		dev_dbg(info->dev, "B calc_ocv present : %d.\n", info->present);
		*ocv = vbatt_avg;
	}
	mutex_unlock(&info->lock);
//	dev_dbg(info->dev, "VBAT average:%d, OCV:%d, IBAT ave:%d, resistor: %d\n", vbatt_avg, *ocv, ibatt_avg, info->resistor);
	return 0;
out:
	return ret;
}
...
//	dev_dbg(info->dev, "calc_soc ocv: %d\n", ocv);
	return 0;
out:
	return ret;
}
...
int pm860x_battery_update_soc(void)
{
	...
	measure_vbatt(info, OCV_MODE_ACTIVE, &vbat);

	dev_dbg(info->dev, "%s,measure_vbatt  : %dmv.\n", __func__, vbat);
