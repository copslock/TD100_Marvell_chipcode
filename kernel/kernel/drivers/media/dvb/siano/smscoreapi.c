...
int smscore_register_hotplug(hotplug_t hotplug)
{
	...
	sms_info("   notifyee %x", (unsigned int)notifyee);

	if (notifyee) {
		/* now notify callback about existing devices */
		first = &g_smscore_devices;
		sms_info(" first %x", (unsigned int)first);
...
int smscore_register_device(struct smsdevice_params_t *params,
	...
	/* alloc common buffer, and will be mmap to user space */
#if ALLOC_COMMON_BUF_MULTIPLE
	/* alloc multi but small common bufs */
	dev->buf_num = params->num_buffers;
#else
	/* alloc a single but large common buf */
	dev->buf_num = 1;
#endif
...
void smscore_onresponse(struct smscore_device_t *coredev,
	...
	if (phdr->msgType != MSG_SMS_DATA_DOWNLOAD_RES)
		sms_debug("RX msg: %d size:%d [%d]\n",
