...
void mmc_wait_for_req(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	do {
		...
		if ((mrq->cmd->error == -EILSEQ)
			 ...
			 */
			if (mmc_host_clk_rate(host) <= 3000000) {
				printk(KERN_WARNING "SD clock is too slow!\n");
				break;
			}

			printk(KERN_WARNING "%s: CRC error! show the clock to %d\n",
					mmc_hostname(host), mmc_host_clk_rate(host)/2);
			mmc_set_clock(host, mmc_host_clk_rate(host)/2);
		} else {
			break;
		}
	} while (1);
