...
static struct resource tps6105x_resources[] = {
	{
		.name = "tps6105x-leds",
	},
...
/*        .name = "tps6105x-leds",*/
		.name = "spotlight",
		.num_resources = 1,
		.resources = &tps6105x_resources[0],
		.id = -1,
	},
	{
		.name = "tps6105x-gpio",
		.num_resources = 1,
		.resources = &tps6105x_resources[1],
		.id = -1,
	},
};
			...
			const struct i2c_device_id *id)
{
	...
	for (i = 0; i < ARRAY_SIZE(tps6105x_cells); i++) {
		...
		tps6105x_cells[i].data_size = sizeof(*tps6105x);
	}

	ret = mfd_add_devices(&client->dev, 0, &tps6105x_cells[0],
		ARRAY_SIZE(tps6105x_cells), &tps6105x_resources[0], 0);
	if (ret)
	{
		printk("tps6105x mfd add devices fail.\n");
		goto fail;
