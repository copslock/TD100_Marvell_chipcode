...
void ext_mono_amp_en(int value)
{
	...
	err = gpio_request(MFP_PIN_GPIO63, "MONO_AMP_EN");
	if (err) {
		gpio_free(MFP_PIN_GPIO63);
		printk(KERN_ERR "Request GPIO failed,"
		       "gpio: %d return :%d\n", MFP_PIN_GPIO63, err);
		return;
	}
	gpio_direction_output(MFP_PIN_GPIO63, value);
	gpio_free(MFP_PIN_GPIO63);
}
					 ...
					 unsigned long arg)
{
	...
	case GPO_EXT_MONO_AMP_STATUS:
		if (machine_is_saar())
			...
			printk(A_DEBUG_MUST "GPO EXT MONO AMP is not being set ==> machine %d does NOT require it!\n", machine_arch_type);
		break;
