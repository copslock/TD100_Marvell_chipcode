...
telephony:
	...
	$(hide)cp $(TELEPHONY)/cp/tavor_td_dkb/Arbel* $(OUTDIR)/telephony/
	$(hide)cp $(TELEPHONY)/cp/tavor_td_dkb/TT* $(OUTDIR)/telephony/
	$(hide)cp $(TELEPHONY)/../rdroot/ramdisk_no_gui.img $(OUTDIR)/telephony
#	$(call my-build-pxafsimage-ext2-target, $(TELEPHONY_DRVSRC)/output/marvell, $(OUTDIR)/telephony/pxafs_ext3.img, pxafs, true, 8192)
	$(shell pwd)/../out/host/linux-x86/bin/make_ext4fs -s -l 8192k -b 1024 -L pxafs $(OUTDIR)/telephony/pxafs_ext4.img $(TELEPHONY_DRVSRC)/output/marvell
	$(log) "td telephony done."
clean_telephony:
	rm -rf $(OUTDIR)/telephony
	$(log) "td telephony cleaned."
