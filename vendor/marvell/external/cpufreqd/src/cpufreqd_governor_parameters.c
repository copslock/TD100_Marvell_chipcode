 ...
 */
static int set_parameter(const unsigned int cpu, const char *governor,
		...
		const struct cpufreq_policy *new_policy, const unsigned int cpu)
{
	...
	while (supported_governors[i] != NULL) {
		if (strcmp(supported_governors[i], new_policy->governor) == 0) {
			...
			ret = set_parameter(cpu, new_policy->governor, parameter, value, is_percentage);
			if (ret == -2 && retry++ < MAX_RETRIES) {
