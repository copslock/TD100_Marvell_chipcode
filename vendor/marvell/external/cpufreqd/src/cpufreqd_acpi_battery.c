...
static int read_battery(struct battery_info *binfo) {
	...
	if (binfo->current_now &&
	    read_int(binfo->current_now, &binfo->present_rate) != 0) {
		clog(LOG_ERR, "Skipping %s\n", binfo->cdev->name);
		return -1;
	}
	if (binfo->energy_now &&
	    read_int(binfo->energy_now, &binfo->remaining) != 0) {
		clog(LOG_ERR, "Skipping %s\n", binfo->cdev->name);
		return -1;
	}
	if (binfo->status && read_value(binfo->status) != 0) {
		clog(LOG_ERR, "Skipping %s\n", binfo->cdev->name);
		return -1;
	}
...
static int open_battery(struct battery_info *binfo) {
	...
	if (binfo->energy_full &&
	    read_int(binfo->energy_full, &binfo->capacity) != 0) {
...
int acpi_battery_update(void) {
	...
	for (i = 0; i < bat_dir_num; i++) {
		...
		if (info[i].present &&
		    read_int(info[i].present, &info[i].is_present) != 0) {
