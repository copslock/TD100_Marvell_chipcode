...
int portq_init(void)
{
	...
	portq_wq = create_workqueue("acipc-msock");
#else
...
struct portq *portq_open(int port)
{
	...
	case CIDATASTUB_PORT:
		...
		portq->rx_q_low_wm = PORTQ_RX_MSG_HUGE_LO_WM;
		portq->rx_q_high_wm = PORTQ_RX_MSG_HUGE_HI_WM;
		break;
...
struct sk_buff *portq_recv(struct portq *portq, bool block)
{
	...
	if (skb &&
		...
		shm_skctl->ap_port_fc = portq_ap_port_fc;
...
static void portq_tx_worker(struct work_struct *work)
{
	...
	while (1) {
		...
		list_for_each_entry_safe(portq, next, &tx_portq_head, tx_list) {
			...
			if (shm_is_xmit_full()) {	/* if ring buffer is full */
				/*
...
static void portq_rx_worker(struct work_struct *work)
{
	...
	for (i = 0; i < PORTQ_RX_WORKQ_SHOTS; i++) {
		...
		if (shm_is_cp_xmit_stopped && shm_has_enough_free_rx_skbuf()) {
