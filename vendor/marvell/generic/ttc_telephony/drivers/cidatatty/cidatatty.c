...
//			tty->ldisc.ops->flush_buffer(tty);
		...
		tty->ldisc.ops->receive_buf(tty, packet, NULL, c);
#else
		tty->ldisc.receive_buf(tty, packet, NULL, c);
...
//	struct tty_struct *tty = cidatatty_table[0];
	...
	while (remain > 0)
	{
		...
		if (c > len)
			c = len;
		//PDEBUG( "data_rx_csd: receive_buf=0x%x\n ",tty->ldisc.receive_buf );
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 32)
		tty->ldisc->ops->receive_buf(tty, packet, NULL, c);
//for N_TTY type 
//		if (tty->ldisc.ops->flush_buffer)
...
static int cidatatty_write(struct tty_struct * tty,     const unsigned char *buffer, int count)
{
	...
	if (writeokflag == 1)
	{
		if (tty_index == PSDTTYINDEX)
			...
			sendCSData(cid, (char *)buffer, count);
		else
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 21)
	cidatatty_tty_driver->devfs_name = "tts/cidatatty%d";
#endif
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 21)
	cidatatty_tty_driver->flags =  TTY_DRIVER_REAL_RAW | TTY_DRIVER_NO_DEVFS;
#else
