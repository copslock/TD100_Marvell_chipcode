...
extern void cp_loadfromflash(void);
extern void cp_loadreliabledatafromflash(void);
extern void cp_releasecp(void);
extern void cp_holdcp(void);

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 36)
static struct pm_qos_request_list cp_qos_disable_cpufreq;
#define CP_QOS_DISABLE_CPUFREQ_PTR (&cp_qos_disable_cpufreq)
#else
static struct pm_qos_request_list *cp_qos_disable_cpufreq;
#define CP_QOS_DISABLE_CPUFREQ_PTR (cp_qos_disable_cpufreq)
#endif

static inline void cp_pm_qos_init(void)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 36)
	pm_qos_add_request(CP_QOS_DISABLE_CPUFREQ_PTR, PM_QOS_CPUFREQ_DISABLE, PM_QOS_DEFAULT_VALUE);
#else
	CP_QOS_DISABLE_CPUFREQ_PTR = pm_qos_add_request(PM_QOS_CPUFREQ_DISABLE, PM_QOS_DEFAULT_VALUE);
#endif
}

static inline void cp_pm_qos_deinit(void)
{
	pm_qos_remove_request(CP_QOS_DISABLE_CPUFREQ_PTR);
}

static inline void cp_pm_qos_disable_cpufreq(void)
{
	pm_qos_update_request(CP_QOS_DISABLE_CPUFREQ_PTR, 1);
}

static inline void cp_pm_qos_enable_cpufreq(void)
{
	pm_qos_update_request(CP_QOS_DISABLE_CPUFREQ_PTR, PM_QOS_DEFAULT_VALUE);
}
 ...
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 21)
void seh_int_handler_high(void *data)
{
...
int seh_api_ioctl_handler(unsigned long arg)
{
	...
	case _EehInsertComm2Reset:
	{
		...
		if(params.params != NULL)
		{
			...
			{
				disable_irq(IRQ_PXA910_CP2_WDT);
			}
		}
		cp_holdcp();
	...
	{
		DBGMSG("Kernel Space _EehCopyCommImageFromFlash Params: No params\n");
...
static int seh_probe(struct platform_device *dev)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 21)
	INIT_WORK(&seh_int_request, seh_int_handler_high, 0);
#else
	...
	ret = request_irq(IRQ_PXA910_CP2_WDT, seh_int_handler_low, IRQF_DISABLED,
			     seh_name, NULL);
	if (ret)
	{
...
static int seh_remove(struct platform_device *dev)
{
	...
	free_irq(IRQ_PXA910_CP2_WDT, NULL);
