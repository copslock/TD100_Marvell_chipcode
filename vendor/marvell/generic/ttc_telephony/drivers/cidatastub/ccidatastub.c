...
extern spinlock_t tx_node_lock;
...

{
	...
	case CCIDATASTUB_GCFDATA:	// For local CGSEND and TGSINK
		if (copy_from_user(&gcfdata, (GCFDATA *) arg, sizeof(GCFDATA)))
			return -EFAULT;
		gcfbuf = kmalloc(gcfdata.len, GFP_KERNEL);
		if (!gcfbuf)
			return -ENOMEM;
		if (copy_from_user(gcfbuf, gcfdata.databuf, gcfdata.len))
		{
			kfree(gcfbuf);
			return -EFAULT;
		}
		sendData(gcfdata.cid, gcfbuf, gcfdata.len);
		kfree(gcfbuf);
		break;
	case CCIDATASTUB_GCFDATA_REMOTE:	// For remote CGSEND and TGSINK
		...
		sendDataRemote(gcfdata.cid, gcfbuf, gcfdata.len);
		kfree(gcfbuf);
		break;
	}
	LEAVE();
