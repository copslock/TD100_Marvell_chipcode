...
#define CITTY_TTY_MINORS                11       /* only have 1 device */
...
#define CCTDEV_NR_DEVS 11        /* ccdev0 through ccdev10 */
		     ...
		     loff_t *f_pos)
{
	...
	tty->ldisc.ops->receive_buf(tty, buf, NULL, c);
#else
	tty->ldisc.receive_buf(tty, buf, NULL, c);
#endif
	up(&sem[minor_num]);
	return c;
}
...
#else
		device_create(cctdev_class, NULL, MKDEV(cctdev_major, cctdev_minor + i), name);
#endif
	}
