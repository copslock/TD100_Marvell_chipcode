...
static const char *readline()
{
	...
	while (p_eol == NULL) {
		...
		rc = select(nfds + 1, &fds, NULL, NULL, NULL);

		if(rc <= 0)
		{
			LOGE("ERROR select fds %d\n" , rc);
		}
		...
		{
			do {
				count = read(pfd[0], &ctrl_msg, sizeof(ctrl_msg));
...
{
	LOGD("Enter reader loop %lu", pthread_self());

	pthread_setname_np(pthread_self(), "ee_readLoop");
...
int at_open(int fd, ATUnsolHandler h)
{
	...
	if(pipe(pfd) < 0)
	{
		LOGD("create pipe error: %s", strerror(errno));
		return -1;
	}

	fcntl(pfd[0], F_SETFL, O_NONBLOCK); //pipe[0] as the read

	if (pthread_mutex_init(&s_commandmutex, NULL) != 0 || pthread_cond_init(&s_commandcond, NULL) != 0)
	{
		return -1;
	}
	...
	LOGD("at_open s_tid_reader is %lu", s_tid_reader);
...
int at_send_command_sync (const char *command, ATCommandType type,
		const char *responsePrefix, const char *smspdu, ATResponse **pp_outResponse)
{
	int err = 0;

	LOGD("at_send_command_sync Enter");


	pthread_mutex_lock(&s_commandmutex);


	LOGD("at_send_command_sync mutex ");

	while (sp_response != NULL)
	{
...
int exit_read_loop_thread(unsigned char cmd)
{
	int ret = -1;
	...
	if(pfd[1] > 0) {
		do
		{
			ret = write(pfd[1], &cmd, sizeof(cmd));
		} while (ret < 0 && errno == EINTR);
		if(ret > 0)
			LOGD("exit read loop thread error %d\n", cmd);
	}
	return ret;
