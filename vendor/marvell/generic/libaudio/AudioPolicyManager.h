...
public:
        ...
        uint32_t getStrategyForStream(AudioSystem::stream_type stream);

        virtual audio_io_handle_t getOutputForEffect(effect_descriptor_t *desc) {return mHardwareOutput;};
        virtual status_t registerEffect(effect_descriptor_t *desc,
                                        audio_io_handle_t output,
                                        uint32_t strategy,
                                        int session,
                                        int id) {return 0;};
        virtual status_t unregisterEffect(int id) {return 0;};
#endif
...
private:
        ...
        bool hasBackMicrophone() const { return false; }
        // true if device is in a telephony/VoIP/VT call
        bool isInCall();
        // true if given state represents a device in a telephony/VoIP/VT call
        bool isStateInCall(int state);
        // true if device is in a VT call
        bool isInVTCall();
        // true if given state represents a device in a VT call
        bool isStateInVTCall(int state);
        ...
        virtual bool needsDirectOuput(AudioSystem::stream_type stream,
                                    ...
                                    AudioSystem::output_flags flags,
                                    uint32_t device);
        virtual uint32_t getMaxEffectsCpuLoad() { return 0; };
        virtual uint32_t getMaxEffectsMemory() { return 0; };


        AudioPolicyClientInterface *mpClientInterface;  // audio policy client interface
        audio_io_handle_t mHardwareOutput;              // hardware output handler
        audio_io_handle_t mA2dpOutput;                  // A2DP output handler
		audio_io_handle_t mPhoneOutput;					// phone plugin output handler
        ...
        static uint32_t mAvailableOutputDevices;                            // bit field of all available output devices
        static uint32_t mAvailableInputDevices;                             // bit field of all available input devices
