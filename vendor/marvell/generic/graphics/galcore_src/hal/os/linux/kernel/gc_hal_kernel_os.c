...
gctBOOL gckOS_ForceMemAllocFail(gckOS Os)
{
    ...
    {
        failMalloc++;
        gcmkPRINT("Total mem Malloc count: %d, force fail Malloc count %d", cntMalloc, failMalloc);
        return gcvTRUE;
    }
    else
    {   
	...
	)
{
    ...
    gcmkVERIFY_ARGUMENT(mdl->pMem != gcvTRUE);
	...
	)
{
	...
	if (mdlMap->vmaAddr == gcvNULL)
	{
		...
		if (mdl->contiguous)
		{
		    ...
		    if(mdl->pMem)
            {
                phyaddr = mdl->dmaHandle;
            }
            else
            {
                phyaddr = virt_to_phys((gctPOINTER)mdl->addr);
            }
	...
	)
{
    ...
    gcmkHEADER_ARG("Os=0x%0x Physsical=%x Bytes=%lu", Os, Physical ,Bytes);
    ...
    if(Os->device->pmemUsage > Os->device->reservedPmemMem)
    {
        gcmkLOG_WARNING_ARGS("Cant's allocate pmem memory for size 0x%08x", (gctUINT32)Bytes);
        return gcvSTATUS_OUT_OF_MEMORY;
    }
    ...
    if (get_pmem_area(PMEM_MINOR_FLAG, region, &pAddr, gcvNULL) < 0)
    {
        gcmkLOG_WARNING_ARGS("Cant's allocate pmem memory for size 0x%08x", (gctUINT32)Bytes);
		gcmkTRACE_ZONE(gcvLEVEL_INFO,
				gcvZONE_OS,
	...
	if (mdl == gcvNULL)
	{
        put_pmem_area(PMEM_MINOR_FLAG, region, gcvNULL);
...
**		Nothing.
*/
gceSTATUS gckOS_FreePmemMemory(
	IN gckOS Os,
	IN gctPHYS_ADDR Physical,
	IN gctSIZE_T Bytes
	)
{
    ...
    for (i = 0; i < mdl->numPages; i++)
	{
		if (mdl->contiguous)
		{
	        ClearPageReserved(virt_to_page((gctPOINTER)(((unsigned long)vaddr) + i * PAGE_SIZE)));
		}
		else
		{
			ClearPageReserved(vmalloc_to_page((gctPOINTER)(((unsigned long)vaddr) + i * PAGE_SIZE)));
		}
    }
#endif

    if(put_pmem_area(PMEM_MINOR_FLAG, mdl->region ,vaddr) < 0)
    {
        gcmkPRINT(" Free Pmem memory failed at region offset %x, region len %x, vaddr %x",
                   mdl->region->offset, mdl->region->len, vaddr);
...
gceSTATUS gckOS_GetPmemPhysicalAddress(
	IN gckOS Os,
	IN gctPHYS_ADDR  Physical,
	OUT gctUINT32 * Address
	)
	...
	)
{
    ...
    {
        /* ref is not zero means signal has been mapped, we just return success and 

           clear the pointer in table, the memory will be freed in unmap function.
        */
        gcmkTRACE_ZONE(gcvLEVEL_ERROR,
    		gcvZONE_OS,
	...
	)
{
	...
	{
		/* The ref is less than 1 means this signal is removed from table and pending to destroy. */
		kfree(signal);
        signal =  gcvNULL;
	}
	...
	)
{
    if(enableClk)
    {
        ...
        if(Frequency != 0)
        {
            ...
            frequency = clk_get_rate(clkGC) / 1000 / 1000;
            if (frequency != Frequency)
            {
                gcmkPRINT("\n[galcore] clk2x input using %d MHz instead of %d MHz, running at %d MHz\n",
                    (int)frequency,
                    (int)Frequency,
                    (int)frequency/2);
            }
            ...
            {
                gcmkPRINT("\n[galcore] clk2x input using %d MHz, running at %d MHz\n",
                    (int)frequency,
                    (int)frequency/2);
            }
        }
        ...
        )
{
    ...
    if(enableDVFM)
    {
        /* disable OP0(100M)and OP1(200M), raise OP2(400M) voltage */
        gc_disable_unsafe_ops(Os->device->dvfm_dev_index);
        /* dvfm_disable_op_name("Video/Graphics M MIPS", Os->device->dvfm_dev_index); */
    }
#endif
	...
	)
{
    ...
    if(enableDVFM)
    {
        /* enable OP0(100M)and OP2(200M), lower OP3(400M) voltage */
        gc_enable_unsafe_ops(Os->device->dvfm_dev_index);
        /* dvfm_enable_op_name("Video/Graphics M MIPS", Os->device->dvfm_dev_index); */
    }
#endif
