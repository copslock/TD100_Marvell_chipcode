...
ulong registerMemBase = 0xc0400000;
    ...
    char __user *buffer, size_t count, loff_t *offset)
{
	...
	len += sprintf(buf+len, "Total video memory can used in Pmem: %ld KB\n", galDevice->reservedPmemMem/1024);
	len += sprintf(buf+len, "Total reserved video memory: %ld KB\n", galDevice->reservedMem/1024);
	
	len += sprintf(buf+len, "Used video mem: %d KB\t pmem video mem: %d KB\tcontiguous: %d KB\tvirtual: %d KB\n", galDevice->vidMemUsage/1024,
							galDevice->pmemUsage/1024,galDevice->contiguousMemUsage/1024, galDevice->virtualMemUsage/1024);

	len += sprintf(buf+len, "Video memory usage in details:\n");
	len += sprintf(buf+len, "Index: %d \tKB\tVertex: %d \tKB\n", galDevice->indexVidMemUsage/1024, galDevice->vertexVidMemUsage/1024);
	len += sprintf(buf+len, "Texture: %d \tKB\tRenderTarget: %d KB\n", galDevice->textureVidMemUsage/1024, galDevice->rendertargetVidMemUsage/1024);
	len += sprintf(buf+len, "Depth: %d \tKB\tBitmap: %d \tKB\n", galDevice->depthVidMemUsage/1024, galDevice->bitmapVidMemUsage/1024);
	len += sprintf(buf+len, "TileStatus: %d \tKB\tMask: %d \tKB\n", galDevice->tilestatusVidMemUsage/1024, galDevice->maskVidMemUsage/1024);
	len += sprintf(buf+len, "Scissor: %d \tKB\tHierarchicalDepth: %d KB\n", galDevice->scissorVidMemUsage/1024, galDevice->hierarchicaldepthVidMemUsage/1024);
	len += sprintf(buf+len, "Others: %d \tKB\n", galDevice->othersVidMemUsage/1024);
...
static long drv_unlocked_ioctl(
                        struct file *filp, unsigned int ioctlCode, unsigned long arg);
#else
...
int drv_open(struct inode *inode, struct file* filp)
{
    ...
    private->tgid               = current->tgid;
    gckOS_MemCopy(private->name, current->comm, TASK_COMM_LEN*sizeof(char));
...
int drv_release(struct inode* inode, struct file* filp)
{
    ...
    gckOS_FreeProcessResource(galDevice->os, private->tgid);
    
	/* Print GC memory usage after every process exits. */
	gcmkPRINT("PID=%d , name=%s exits\n", private->tgid, private->name);
   
	gcmkPRINT("GC memory usage profile:\n");
	/* gcmkPRINT("Total reserved video memory: %ld KB\n", galDevice->reservedMem/1024);*/
	gcmkPRINT("Used video mem: %d KB\t Used Pmem mem: %d KB\tcontiguous: %d KB\tvirtual: %d KB\n", galDevice->vidMemUsage/1024,
							galDevice->pmemUsage/1024,galDevice->contiguousMemUsage/1024, galDevice->virtualMemUsage/1024);
	gcmkPRINT("Video memory usage in details:\n");
	gcmkPRINT("Index: %d \tKB\tVertex: %d \tKB\n", galDevice->indexVidMemUsage/1024, galDevice->vertexVidMemUsage/1024);
	gcmkPRINT("Texture: %d \tKB\tRenderTarget: %d KB\n", galDevice->textureVidMemUsage/1024, galDevice->rendertargetVidMemUsage/1024);
	gcmkPRINT("Depth: %d \tKB\tBitmap: %d \tKB\n", galDevice->depthVidMemUsage/1024, galDevice->bitmapVidMemUsage/1024);
	gcmkPRINT("TileStatus: %d \tKB\tMask: %d \tKB\n", galDevice->tilestatusVidMemUsage/1024, galDevice->maskVidMemUsage/1024);
	gcmkPRINT("Scissor: %d \tKB\tHierarchicalDepth: %d KB\n", galDevice->scissorVidMemUsage/1024, galDevice->hierarchicaldepthVidMemUsage/1024);
	gcmkPRINT("Others: %d \tKB\n", galDevice->othersVidMemUsage/1024);
...
#endif
{
...
#if VIDEO_MEMORY_USE_PMEM
    /*!FIXME: Temporary hack for insmode control the pmem size
    pmemSize = contiguousSize;
    contiguousSize = 8 << 20;*/
#endif
	...
	if (showArgs)
	{
		...
		gcmkPRINT("  registerMemSize = %08lx\n",   registerMemSize);
		gcmkPRINT("  contiguousSize  = %08lx\n",   contiguousSize);
