...
int guard_thread(void *ctxt)
{
    ...
    gctBOOL isIdle;
    gceCHIPPOWERSTATE chipPowerState = gcvPOWER_ON;
    gctUINT32  pixel_counter_old =0;
    gctUINT32  pixel_counter_new =0;
    while (1)
	{
        delayTime = device->profileStep; 
         ...
         && (device->kernel->atomClients > 0))
	    {
            ...
            chipPowerState = device->kernel->hardware->chipPowerState;

            if (chipPowerState == gcvPOWER_OFF || chipPowerState == gcvPOWER_SUSPEND)
            {
                count = 0;
                captureAddr = 0xFFFFFFFF;
                currentAddr = 0xFFFFFFFF;
                gcmkVERIFY_OK(gckOS_ReleaseRecMutex(device->os, device->kernel->hardware->recMutexPower));
                gcmkONERROR(gckOS_Delay(device->os, delayTime));
                continue;
            }
            ...
            if(device->clkEnabled)
            {
                ...
                if(count >= 3)
                {
                    gcmkONERROR(gckOS_WriteRegister(device->os, 0x00470, (  ((((gctUINT32) (0)) & ~(((gctUINT32) (((gctUINT32) ((((1 ? 27:24) - (0 ? 27:24) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 27:24) - (0 ? 27:24) + 1))))))) << (0 ? 27:24))) | (((gctUINT32) ((gctUINT32) (8) & ((gctUINT32) ((((1 ? 27:24) - (0 ? 27:24) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 27:24) - (0 ? 27:24) + 1))))))) << (0 ? 27:24))) )));
                    gcmkONERROR(gckOS_ReadRegister(device->os, 0x0045C, &pixel_counter_old));
                    gcmkONERROR(gckOS_Delay(device->os, 300));
                    gcmkONERROR(gckOS_WriteRegister(device->os, 0x00470, (  ((((gctUINT32) (0)) & ~(((gctUINT32) (((gctUINT32) ((((1 ? 27:24) - (0 ? 27:24) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 27:24) - (0 ? 27:24) + 1))))))) << (0 ? 27:24))) | (((gctUINT32) ((gctUINT32) (8) & ((gctUINT32) ((((1 ? 27:24) - (0 ? 27:24) + 1) == 32) ? ~0 : (~(~0 << ((1 ? 27:24) - (0 ? 27:24) + 1))))))) << (0 ? 27:24))) )));
                    gcmkONERROR(gckOS_ReadRegister(device->os, 0x0045C, &pixel_counter_new));
                    ...
                    if(pixel_counter_new != pixel_counter_old)
                    {
                        gcmkPRINT("Scilent Reset disable .Because shader pixel_count is still on move\n: new_cnt:0x%x ,old_cnt:0x%x\n",
                            pixel_counter_new, pixel_counter_old);
                        needSilentReset = gcvFALSE;
    ...
    )
{
    ...
    gcmkPRINT("\n[galcore] registerBase =0x%08x, registerMemSize = 0x%08x, contiguousBase= 0x%08x, contiguousSize = 0x%08x, pmemSize = 0x%08x\n", 
              (gctUINT32)RegisterMemBase, (gctUINT32)RegisterMemSize, (gctUINT32)ContiguousBase, (gctUINT32)ContiguousSize, (gctUINT32)PmemSize);
    ...
    device->kernel->bDisablePmem = (PmemSize == 0);
...
//			device->contiguousBase     = (gctPOINTER) ioremap_nocache(ContiguousBase, ContiguousSize);
    ...
    gcmkPRINT("\n[galcore] GC use Pmem as video MM is limited to Size = 0x%08x \n",  (gctUINT32)(device->reservedPmemMem));
