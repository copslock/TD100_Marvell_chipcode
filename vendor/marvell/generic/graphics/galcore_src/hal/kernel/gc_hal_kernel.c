...
**  
**  The rule is the non-aligned bytes take the ratio of total size. 
**
**  current ration is 10%, if watste bytes larger than 10% of total size, we use
**
...
{\
    status = gckVIDMEM_AllocateLinear(videoMemory, Bytes, Alignment, Type, Node);\
	if (gcmIS_SUCCESS(status))\
	{\
	...
	)
{
    ...
    if(gckOS_ForceMemAllocFail(Kernel->os))
    {
        gcmkLOG_WARNING_ARGS("Force Memory allocate fail, Bytes=%d, Alignment=%d, Type=%d, Node=%p",
            Bytes, Alignment, Type, Node);
    ...
    do
	{
        ...
        if(_UsePmem(Bytes))
        {
            /* Try allocate memory from pmem first. */
            gckVIDMEM_AllocateMemory(gcvTRUE, gcvTRUE);

            /* Try allocate memory from reserve if pmem failed */
            gckVIDMEM_AllocateReserve();
        }
        else
        {
            /* Try allocate memory from reserve first */
            gckVIDMEM_AllocateReserve();

            /* Try allocate memory from pmem if reserved failed */
            gckVIDMEM_AllocateMemory(gcvTRUE, gcvTRUE);
        }

        /* Try allocate memory from continues memory if above allocate failed */
        gckVIDMEM_AllocateMemory(gcvTRUE, gcvFALSE);

        /* Try allocate memory from vmalloc if above allocate failed */
	...
	if (gcmIS_SUCCESS(status))
	{
		...
		gckOS_UpdateVidMemUsage(Kernel->os, gcvTRUE, Bytes, Type);
	}
    else
    {
        gcmkLOG_ERROR_ARGS("_AllocateMemory failed, Type %d, Pool %d, Byte %d Use Pmem %d",
	...
	)
{
	...
	do
	{
...
#ifdef __QNXNTO__
			gcmkERR_BREAK(
				gckVIDMEM_ConstructVirtual(Kernel, gcvFALSE, gcvFALSE, Bytes, Handle, Node));
#else
			gcmkERR_BREAK(
				gckVIDMEM_ConstructVirtual(Kernel, gcvFALSE, gcvFALSE, Bytes, Node));
#endif
			/* Save surface type. */
			(*Node)->Virtual.type = Type;
			/* Success. */
        ...
        else if (pool == gcvPOOL_PMEM)
        {
			/* Create a gcuVIDMEM_NODE for pmem memory. */
#ifdef __QNXNTO__
			status = gckVIDMEM_ConstructVirtual(Kernel, gcvTRUE, gcvTRUE, Bytes, Handle, Node);
#else
			status = gckVIDMEM_ConstructVirtual(Kernel, gcvTRUE, gcvTRUE, Bytes, Node);
#endif
			if (gcmIS_SUCCESS(status))
			{
				/* Save surface type. */
...
#ifdef __QNXNTO__
			status = gckVIDMEM_ConstructVirtual(Kernel, gcvTRUE, gcvFALSE, Bytes, Handle, Node);
#else
			status = gckVIDMEM_ConstructVirtual(Kernel, gcvTRUE, gcvFALSE, Bytes, Node);
#endif
			if (gcmIS_SUCCESS(status))
			{
				/* Save surface type. */
...
#if VIDEO_MEMORY_USE_PMEM            
		{
            /* Advance to pmem memory. */
			pool = gcvPOOL_PMEM;
		}
        else
		if (pool == gcvPOOL_PMEM)
#endif            
		{
