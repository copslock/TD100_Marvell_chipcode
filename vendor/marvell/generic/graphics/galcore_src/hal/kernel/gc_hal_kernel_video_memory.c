    ...
    )
{
    ...
    {
		/*gcmkLOG_WARNING_ARGS("Not enough reserved video memory, Type=%d, Bytes=%d, Memory->freeBytes=%d", Type, Bytes, Memory->freeBytes);*/
        /* Not enough memory. */
    ...
    *Node = node;
	...
	gckOS_UpdateVidMemUsage(Memory->os, gcvTRUE, node->VidMem.bytes, gcvSURF_NUM_TYPES);
    ...
    )
{
    ...
    gctBOOL acquired = gcvFALSE;
	gctSIZE_T bytes = 0;
	gceSURF_TYPE surType = gcvSURF_TYPE_UNKNOWN;
	gckOS os = gcvNULL;
    gcmkHEADER_ARG("Node=0x%x", Node);
    ...
    if (Node->VidMem.memory->object.type == gcvOBJ_VIDMEM)
    {
		...
		/* Update video memory usage. */
		gckOS_UpdateVidMemUsage(memory->os, gcvFALSE, bytes, gcvSURF_NUM_TYPES);

		/* Update video memory usage according to surface type. */
		gckOS_UpdateVidMemUsage(memory->os, gcvFALSE, bytes, surType);
    ...
    {
        if(Node->Virtual.pmem)
        {
        	/* Free the pmem memory */
        	gcmkVERIFY_OK(status = gckOS_FreePmemMemory(Node->Virtual.kernel->os, 
                                     Node->Virtual.physical, 
                                     Node->Virtual.bytes));
        }
        else
        {
			...
			gcmkVERIFY_OK(status = gckOS_FreePagedMemory(Node->Virtual.kernel->os,
                                            Node->Virtual.physical,
                                            Node->Virtual.bytes));
        }
		...
		/* Update video memory usage according to surface type. */
		if (os && status == gcvSTATUS_OK)
			gckOS_UpdateVidMemUsage(os, gcvFALSE, bytes, surType);
    }
