...
OSCL_EXPORT_REF int16 mrvl_video_config_parser(pvVideoConfigParserInputs *aInputs, pvVideoConfigParserOutputs *aOutputs)
{
    if (aInputs->iMimeType == PVMF_MIME_M4V) //m4v
    {
        ...
        int16 retval = 0;
        retval = iMRVLDecodeVOLHeader(&psBits, &width, &height, &display_width, &display_height, &profile_level);
        if (retval != 0)
        {
            return retval;
        }
