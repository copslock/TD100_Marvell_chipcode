...
#define MRVL_H264_MASK_SUPPORTCABAC	0x00000001
#define MRVL_H264_MASK_NOTFRAMEMBONLY	0x00000002
...
OSCL_EXPORT_REF int16 iGetM4VConfigInfo(uint8 *buffer, int32 length, int32 *width, int32 *height, int32 *display_width, int32 *display_height)
{
    ...
    *width = *height = *display_height = *display_width = 0;
    ...
    status = iMRVLDecodeVOLHeader(&psBits, width, height, display_width, display_height, &profilelevel);
    return status;
}
...
// return:  error code
OSCL_EXPORT_REF int16 iMRVLDecodeVOLHeader(mp4StreamType *psBits, int32 *width, int32 *height, int32 *display_width, int32 *display_height, int32 *profilelevel)
{
    int16 iErrorStat;
    uint32 codeword;
    ...
    if (codeword == VO_START_CODE)
    {
        ...
        if (codeword != VOL_START_CODE)
        {
            ...
            if (codeword == 0x20){
                ...
                /*source_format*/                 
                err = ReadBits(psBits, 3, &codeword);
                if (err == -2){
                    /*there no data left in the header, set resolution to 176*144 by default. */
                    *width = 176;
                    *height = 144;
                    *display_width = *width;
...
int16 DecodeSPS(mp4StreamType *psBits, int32 *width, int32 *height, int32 *display_width, int32 *display_height, int32 *profile_idc, int32 *level_idc)
{
    ...
    *level_idc = temp;
    ...
    if (*profile_idc == 100 || *profile_idc == 110 || *profile_idc == 122 || *profile_idc == 144) {
        ...
        ue_v(psBits, &temp); /*chroma_format_idc*/
       if (temp == 3) {
           ReadBits(psBits, 1, &temp); /*residual_colour_transform_flag*/
       }
       ue_v(psBits, &temp);/*bit_depth_luma_minus8*/
       ue_v(psBits, &temp); /*bit_depth_chroma_minus8*/
       ReadBits(psBits, 1, &temp); /*qpprime_y_zero_transform_bypass_flag*/
       ReadBits(psBits, 1, &temp); /*seq_scaling_matrix_present_flag*/
       if (temp) {
           for(i=0;i<8;i++){
                ReadBits(psBits, 1, &temp); /*seq_scaling_list_present_flag*/
           }
       }
    }
    ue_v(psBits, &temp);
    ...
    *display_height = *height = (temp + 1) << 4;
	...
	property_get("ro.mrvl.h264.dec.cap", value, "0");
	if (!(atoi(value) & MRVL_H264_MASK_NOTFRAMEMBONLY))
	{
            return MP4_INVALID_VOL_PARAM;
	}
        //ReadBits(psBits,1, &temp);
...
int32 DecodePPS(mp4StreamType *psBits)
{
    ...
    if (entropy_coding_mode_flag)
    {
        char value[PROPERTY_VALUE_MAX];
	property_get("ro.mrvl.h264.dec.cap", value, "0");
	if (!(atoi(value) & MRVL_H264_MASK_SUPPORTCABAC))
            return 1;
    }
