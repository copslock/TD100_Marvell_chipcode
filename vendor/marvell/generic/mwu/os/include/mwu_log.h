...
#define mwu_printf(level, ...) do { if (level & mwu_debug_level) LOGD(__VA_ARGS__); } while(0)
...
void mwu_hexdump(int level, const char *title, const unsigned char *buf,
                 size_t len);
/** ENTER print */
#define ENTER()         mwu_printf(DEBUG_CALL_FLOW, "Enter: %s, %s:%i", __FUNCTION__, \
                            __FILE__, __LINE__)
/** LEAVE print */
#define LEAVE()         mwu_printf(DEBUG_CALL_FLOW, "Leave: %s, %s:%i", __FUNCTION__, \
                            __FILE__, __LINE__)

#else /* STDOUT_DEBUG */

/** WPS printf */
#define mwu_printf(args...) do { } while (0)
/** WPS hexdump */
#define mwu_hexdump(args...) do { } while (0)
