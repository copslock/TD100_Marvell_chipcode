...
static int str_to_argv(char *str, int *argc, char ***argv)
{
    ...
    *argc = 0;
    ...
    while (1) {
        switch (str[i]) {
        case '\0':
            /* this is the end of the string.  Process it like usual, but
             * quit
             */
        case ' ':
            /* Just finished parsing an arg.  Terminate it and add it to
             * the argv list
             */
            c = str[i];
            str[i] = 0;
        ...
        case '"':
            /* This is the beginning of a quoted string.  Find the end. */
            i++;
            while (str[i] != 0 && str[i] != '"') {
                i++;
            }
...
static void cli(void)
{
    ...
    while (1) {
        ...
        if (FD_ISSET(STDIN_FILENO, &rdset)) {
            ret = read(STDIN_FILENO, &cmdline[end], sizeof(cmdline) - end);
            if (ret == -1) {
