 ...
 XMODEM_API int pollUart(HANDLE hPort, unsigned char targetByte, int TimeOut)
 {
	...
	int found=0;
		...
		do
			{
			//	time(&NowTime);
  				ReadBytes(hPort, Buffer, 1, &ByteRead);
				NowTime = GetTickCount();
				
//				printf("Polling for %X (%d:%d) %d  %X\n",targetByte, TimeOut, (NowTime-StartTime), ByteRead, Buffer[0]);
				
 				if(ByteRead && Buffer[0] == targetByte){found=1; break;}
...
XMODEM_API int XModemVersion()
{
	return MFG_VERSION(XMODEMCTRL_VERSION_MAJOR1, XMODEMCTRL_VERSION_MAJOR2, 
		XMODEMCTRL_VERSION_MINOR1,XMODEMCTRL_VERSION_MINOR2);

}

XMODEM_API char* XModemAbout()
{
	...
	sprintf(AboutString, AboutFormat, 
			XMODEMCTRL_VERSION_MAJOR1,	
			XMODEMCTRL_VERSION_MAJOR2,
			XMODEMCTRL_VERSION_MINOR1, 
			XMODEMCTRL_VERSION_MINOR2,
			__DATE__, __TIME__);
