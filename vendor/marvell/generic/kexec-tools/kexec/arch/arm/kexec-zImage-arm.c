	...
	struct kexec_info *info)
{
	...
	char *modified_cmdline = NULL, *tmp_buf;
	...
	while((opt = getopt_long(argc, argv, short_options, options, 0)) != -1) {
		...
		case OPT_TIMESTAMP:
			...
			sscanf(ver, "%*[^ ] %*[^ ] %*[^ ] %s %d %*[^ ]"
				       " %*[^ ] %d", month, &date, &year);
			snprintf(timestamp, 10, "%4d%3s%2d", year, month, date);
			timestamp[9] = 0;
			break;
		}
	}
	...
	if (info->kexec_flags & KEXEC_ON_CRASH) {
		...
		tmp_buf = xmalloc(COMMAND_LINE_SIZE);
		if (!modified_cmdline || !tmp_buf)
			return -1;
		...
		if (timestamp[0] != 0) {
			command_line_len = strlen(modified_cmdline);
			sprintf(tmp_buf, "%s kdump_ver=%s", modified_cmdline,
				timestamp);
			command_line_len += 20;
			strncpy(modified_cmdline, tmp_buf, command_line_len);
			modified_cmdline[command_line_len] = '\0';
		}
